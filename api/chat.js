...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (error instanceof OpenAI.APIError) {
      if (error.status === 429) {
        return res.status(429).json({ error: "I'm currently experiencing high demand. Please try again in a little while." });
      } else {
        return res.status(error.status || 500).json({ error: "Sorry, I couldn't fetch my info right now. Try asking again in a moment!" });
      }
    } else if (error.code === 'ENOTFOUND') {
      return res.status(500).json({ error: 'Could not connect to OpenAI. Please check your network connection.' });
    } else {
      return res.status(500).json({ error: 'An internal server error occurred. This could be due to an invalid API key or a problem with the OpenAI service.' });
    }
  }
}
